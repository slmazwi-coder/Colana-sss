...
const get = (key, fallback) => {
  try {
    const raw = localStorage.getItem(`css_${key}`)
    return raw ? JSON.parse(raw) : fallback
  } catch { return fallback }
}

const set = (key, value) => {
  try { localStorage.setItem(`css_${key}`, JSON.stringify(value)) } catch {}
}

// ── School constants ────────────────────────────────────────────────────────
export const SCHOOL = {
  name:       'Colana Senior Secondary School',
  short:      'Colana SSS',
  motto:      'Vuka Ulwe Uqaqambe',
  emis:       '',
  ...
  facebook:   '#',
  tiktok:     'https://www.tiktok.com/@yolls41',
}
...
const DEFAULT_NEWS = [
  ...
  },
  {
    ...
    body: 'Colana SSS celebrated the Class of 2026 with a memorable tie ceremony on 26 March 2026. Learners received their custom-designed matric ties, symbolising their final year of schooling. The event was attended by staff, learners, parents, and community members.',
    image: '/assets/ceremony.jpg',
    category: 'Events',
  },
  {
    id: '3',
    title: 'Backpack Donation Drive',
    date: '2026-02-15',
    excerpt: 'Colana SSS learners received brand new school backpacks through a generous community donation.',
    body: 'A heartfelt community donation drive brought brand new school backpacks to our learners. This initiative demonstrates the strong bond between Colana SSS and its surrounding community. We thank all donors for their generosity.',
...
const DEFAULT_ABOUT = {
  history: [
    'Colana Senior Secondary School is a public secondary school serving learners from Grade 8 to Grade 12 in the Eastern Cape, South Africa.',
    ...
    'With our motto "Vuka Ulwe Uqaqambe" (Rise Up and Progress), Colana SSS is committed to academic excellence, community values, and the holistic development of every learner who walks through our doors.',
  ...
  principal: {
    ...
    title: 'Principal',
    message: [
      'Welcome to Colana Senior Secondary School. We believe every learner carries within them the capacity for greatness. Our role is to unlock it — through discipline, love, and unwavering belief in their potential.',
      'At Colana SSS, we value respect, responsibility, and pride in our school community. Our motto — Vuka Ulwe Uqaqambe — inspires us to rise up each day and pursue progress in everything we do.',
    ],
  },
}
...
// ── Auth ────────────────────────────────────────────────────────────
export const isAuthenticated = () => localStorage.getItem('css_auth') === 'true'
export const login  = (pw) => { if (pw === 'admin2026') { localStorage.setItem('css_auth', 'true'); return true } return false }
export const logout = ()   => localStorage.removeItem('css_auth')

// ── IDs ────────────────────────────────────────────────────────────
export const generateId = () => `${Date.now().toString(36)}-${Math.random().toString(36).slice(2, 7)}`

export const generateStudentNumber = (year) => {
  const key = `css_ctr_${year}`
